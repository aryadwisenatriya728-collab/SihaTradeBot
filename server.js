...
// ── RESEND EMAIL ──────────────────────────────────────────
async function sendOTPviaResend(toEmail, otp) {
  const apiKey = process.env.RESEND_API_KEY;
  if (!apiKey) throw new Error('RESEND_API_KEY belum diset di Railway.');

  const body = JSON.stringify({
    from: 'SihaTradeBot <onboarding@resend.dev>',
    to: [toEmail],
  ...
  return new Promise((resolve, reject) => {
    const req = https.request({
      ...
      res.on('end', () => {
        ...
        else reject(new Error(json.message || 'Gagal kirim email.'));
      });
    });
    req.on('error', reject);
    req.write(body);
...
// ── AUTH ───────────────────────────────────────────────────
// Step 2: Verifikasi OTP + buat akun (OTP diverifikasi di frontend via EmailJS)
app.post('/api/auth/register', (req, res) => {
  ...
  if (otp !== 'VERIFIED') return res.json({ ok:false, msg:'Verifikasi OTP gagal.' });
...
// Reset password dengan OTP (OTP diverifikasi di frontend via EmailJS)
