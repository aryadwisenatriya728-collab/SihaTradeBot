...
// ── FONNTE WHATSAPP OTP ───────────────────────────────────
async function sendOTPviaBrevo(toPhone, otp) {
  const token = process.env.FONNTE_TOKEN;
  if (!token) throw new Error('FONNTE_TOKEN belum diset di environment variable.');
  console.log('[OTP] Mengirim ke', toPhone, 'via WhatsApp...');
  const body = new URLSearchParams({
    target: toPhone,
    message: `🤖 *SihaTradeBot*\n\nKode OTP kamu:\n*${otp}*\n\nBerlaku 10 menit. Jangan share ke siapapun.`,
    countryCode: '62'
  });
  ...
  console.log('[OTP] Fonnte response:', JSON.stringify(result));
  if (!result.status) throw new Error('Fonnte error: ' + (result.reason || JSON.stringify(result)));
  console.log('[OTP] WhatsApp OTP berhasil dikirim ke', toPhone);
}
...
app.post('/api/auth/send-otp', async (req, res) => {
  const { email } = req.body; // email field dipakai untuk nomor WA / email (identifier)
  if (!email) return res.json({ ok:false, msg:'Nomor WhatsApp tidak valid.' });
  const users = loadUsers();
  if (users[email]) return res.json({ ok:false, msg:'Nomor sudah terdaftar.' });
  const otp = generateOTP();
  otpStore.set(email, { code:otp, expiresAt:Date.now() + 10*60*1000 });
  try {
    await sendOTPviaBrevo(email, otp);
    res.json({ ok:true, msg:'Kode OTP dikirim ke WhatsApp.' });
  } catch(e) { res.json({ ok:false, msg:'Gagal kirim OTP: ' + e.message }); }
});
...
app.post('/api/auth/forgot-otp', async (req, res) => {
  ...
  try {
    await sendOTPviaBrevo(email, otp);
    res.json({ ok:true, msg:'Kode OTP reset dikirim ke WhatsApp.' });
  } catch(e) { res.json({ ok:false, msg:'Gagal kirim OTP: ' + e.message }); }
});
