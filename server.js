...
async function sendOTPviaBrevo(toEmail, otp) {
  ...
  if (!apiKey) throw new Error('BREVO_SMTP_KEY belum diset di Railway.');
  console.log('[OTP] Mengirim ke', toEmail, 'via Brevo API...');
  const body = JSON.stringify({
    sender: { name: 'SihaTradeBot', email: 'a9de1d001@smtp-brevo.com' },
    to: [{ email: toEmail }],
  ...
  return new Promise((resolve, reject) => {
    const req = https.request({
      ...
      res.on('end', () => {
        console.log('[OTP] Brevo response:', res.statusCode, d);
        if (res.statusCode >= 200 && res.statusCode < 300) resolve(true);
        else reject(new Error('Brevo error ' + res.statusCode + ': ' + d));
      });
    });
    req.on('error', (e) => { console.log('[OTP] Brevo error:', e.message); reject(e); });
    req.write(body);
    req.end();
  });
}
...
app.post('/api/auth/send-otp', async (req, res) => {
  ...
  } catch(e) { res.json({ ok:false, msg:'Gagal kirim email: ' + e.message }); }
});
