...
// ============================================================
// KRONOS CLIENT — via ngrok URL (Flask di Google Colab)
// ============================================================
// Set KRONOS_URL di Railway Variables dengan URL ngrok dari Colab
// Contoh: https://xxxx-xx-xx.ngrok-free.app
// ============================================================

const KRONOS_URL = process.env.KRONOS_URL || '';

function kronosRequest(path, body) {
  return new Promise((resolve, reject) => {
    if (!KRONOS_URL) return reject(new Error('KRONOS_URL belum diset di Railway'));
    const bodyStr = JSON.stringify(body);
    try {
      const url = new URL(KRONOS_URL + path);
      ...
      const req = lib.request(options, res => {
        ...
        res.on('end', () => {
          try { resolve(JSON.parse(data)); }
          catch (e) { reject(new Error('Response bukan JSON: ' + data.slice(0,100))); }
        });
      });
      req.setTimeout(30000, () => { req.destroy(); reject(new Error('Kronos timeout')); });
      req.on('error', reject);
      req.write(bodyStr);
      req.end();
    } catch(e) { reject(e); }
...
async function kronosConfirmEntry(bitget, market) {
  if (!KRONOS_URL) return { ok: false, error: 'KRONOS_URL belum diset' };
  try {
    const candleRes = await bitget.getCandles(market.symbol, market.timeframe, 200);
    const rawCandles = candleRes.data || [];
    if (rawCandles.length < 30) return { ok: false, error: 'Data candle kurang' };
    const candles = rawCandles
      .map(c => ({ time: Math.floor(parseInt(c[0])/1000), open: parseFloat(c[1]),
                   high: parseFloat(c[2]), low: parseFloat(c[3]),
                   close: parseFloat(c[4]), volume: parseFloat(c[5]||0) }))
      .sort((a,b) => a.time - b.time);
    const result = await kronosRequest('/predict', { symbol: market.symbol, candles, pred_len: 5 });
    if (!result.ok) return { ok: false, error: result.error || 'Kronos predict gagal' };
    return { ok: true, ...result };
  } catch (e) { return { ok: false, error: 'Kronos error: ' + e.message }; }
}

async function kronosExitDecision(bitget, position, market) {
  if (!KRONOS_URL) return { action: 'HOLD', reason: 'KRONOS_URL belum diset', confidence: 50 };
  try {
    const candleRes = await bitget.getCandles(market.symbol, market.timeframe || '5m', 100);
    const rawCandles = candleRes.data || [];
    if (rawCandles.length < 20) return { action: 'HOLD', reason: 'Data candle kurang', confidence: 50 };
    const candles = rawCandles
      .map(c => ({ time: Math.floor(parseInt(c[0])/1000), open: parseFloat(c[1]),
                   high: parseFloat(c[2]), low: parseFloat(c[3]),
                   close: parseFloat(c[4]), volume: parseFloat(c[5]||0) }))
      .sort((a,b) => a.time - b.time);
    const result = await kronosRequest('/exit', {
      symbol: market.symbol, candles,
      position: { side: position.side, entry: position.entry,
                  markPrice: position.markPrice, unrealizedPL: position.unrealizedPL }
    });
    if (!result.ok) return { action: 'HOLD', reason: 'Kronos error', confidence: 50 };
    return { action: result.action, reason: result.reason, confidence: result.confidence };
  } catch (e) { return { action: 'HOLD', reason: 'Kronos timeout: ' + e.message, confidence: 50 }; }
}
...
class NexusBot {
  ...
  // ── Cek apakah Kronos ngrok service tersedia ──
  async checkKronosHealth() {
    if (!KRONOS_URL) {
      this.kronosAvailable = false;
      this.log('SYS', 'Kronos: ⚠️ KRONOS_URL belum diset — fallback ke Gemini');
      return;
    }
    try {
      const result = await kronosRequest('/health', {});
      this.kronosAvailable = !!result.ok;
      this.log('SYS', `Kronos (Colab/ngrok): ${result.ok ? '✅ ONLINE model=' + result.model : '❌ ERROR'}`);
    } catch (e) {
      this.kronosAvailable = false;
      this.log('SYS', `Kronos: ⚠️ Tidak terjangkau — ${e.message} | Pastikan Colab masih jalan`);
    }
  }
