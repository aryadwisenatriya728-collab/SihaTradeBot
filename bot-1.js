...
// ============================================================
// NEXUS BOT — Bitget Futures + Kronos AI + Gemini Q&A
// ============================================================
// AI ROLE SPLIT:
//   Kronos (Python microservice :7000) → konfirmasi entry + keputusan exit
//   Gemini                             → chat Q&A + explain signal (tetap)
// ============================================================
...
// ============================================================
// KRONOS CLIENT — via HuggingFace Inference API (tanpa Python)
// ============================================================
// Setup: tambah HUGGINGFACE_TOKEN di environment variable Railway/Render
// Dapet token gratis di: https://huggingface.co/settings/tokens
// ============================================================

const HF_MODEL = 'NeoQuasar/Kronos-small';
const HF_TOKEN = process.env.HUGGINGFACE_TOKEN || '';

/**
 * Hit HuggingFace Inference API untuk Kronos
 * Input: array of OHLCV numbers (close prices saja, HF endpoint time-series)
 */
function kronosHFRequest(inputs) {
  return new Promise((resolve, reject) => {
    ...
    req.setTimeout(25000, () => { req.destroy(); reject(new Error('HF timeout')); });
    req.on('error', reject);
    req.write(body);
    req.end();
  });
...
/**
 * Kronos: konfirmasi entry signal via HuggingFace
 * Ambil candle → kirim close prices → analisis forecast
 * Return: { ok, signal, confidence, takeProfit, stopLoss, reasoning }
 */
async function kronosConfirmEntry(bitget, market) {
  if (!HF_TOKEN) {
    return { ok: false, error: 'HUGGINGFACE_TOKEN belum diset' };
  }
  try {
    const candleRes = await bitget.getCandles(market.symbol, market.timeframe, 200);
    const rawCandles = candleRes.data || [];
    if (rawCandles.length < 30) {
      return { ok: false, error: 'Data candle kurang' };
    }

    // Kronos input: array close prices ascending
    const closes = rawCandles
      .map(c => parseFloat(c[4]))
      .reverse()
      .slice(-100); // max 100 candle supaya cepat

    const currentClose = closes[closes.length - 1];

    const res = await kronosHFRequest(closes);

    // Model loading (503) → tunggu, return fallback
    if (res.status === 503) {
      return { ok: false, error: 'Kronos model sedang loading di HF, coba lagi 30 detik' };
    }
    if (res.status !== 200) {
      return { ok: false, error: `HF API error ${res.status}: ${JSON.stringify(res.body).slice(0, 100)}` };
    }

    // HF response: array of predicted values
    const predicted = Array.isArray(res.body) ? res.body : (res.body?.generated_text ? [] : null);
    if (!predicted || predicted.length === 0) {
      return { ok: false, error: 'Response Kronos kosong atau format tidak dikenal' };
    }

    // Ambil 5 prediksi pertama sebagai forecast close prices
    const predCloses = predicted.slice(0, 5).map(v => typeof v === 'number' ? v : parseFloat(v));
    const lastPred   = predCloses[predCloses.length - 1];
    const changePct  = (lastPred - currentClose) / currentClose * 100;

    // Derive signal dari arah forecast
    let signal, confidence, takeProfit, stopLoss, reasoning;

    if (changePct >= 0.4) {
      signal     = 'LONG';
      confidence = Math.min(92, Math.round(60 + Math.abs(changePct) * 14));
      takeProfit = currentClose * (1 + Math.max(changePct / 100, 0.006));
      stopLoss   = currentClose * (1 - Math.max(Math.abs(changePct) / 200, 0.004));
      reasoning  = `Kronos prediksi naik ~${changePct.toFixed(2)}% ($${currentClose.toFixed(4)} → $${lastPred.toFixed(4)})`;
    } else if (changePct <= -0.4) {
      signal     = 'SHORT';
      confidence = Math.min(92, Math.round(60 + Math.abs(changePct) * 14));
      takeProfit = currentClose * (1 - Math.max(Math.abs(changePct) / 100, 0.006));
      stopLoss   = currentClose * (1 + Math.max(Math.abs(changePct) / 200, 0.004));
      reasoning  = `Kronos prediksi turun ~${Math.abs(changePct).toFixed(2)}% ($${currentClose.toFixed(4)} → $${lastPred.toFixed(4)})`;
    } else {
      signal     = 'WAIT';
      confidence = 50;
      takeProfit = 0;
      stopLoss   = 0;
      reasoning  = `Kronos prediksi sideways (Δ ${changePct.toFixed(2)}%) — hindari entry`;
    }
    ...
    return { ok: false, error: 'Kronos error: ' + e.message };
  }
}

/**
 * Kronos: keputusan exit posisi terbuka via HuggingFace
 * Return: { action: "CLOSE"|"HOLD", reason, confidence }
 */
async function kronosExitDecision(bitget, position, market) {
  if (!HF_TOKEN) {
    return { action: 'HOLD', reason: 'HUGGINGFACE_TOKEN belum diset', confidence: 50 };
  }
  try {
    const candleRes = await bitget.getCandles(market.symbol, market.timeframe || '5m', 100);
    const rawCandles = candleRes.data || [];
    if (rawCandles.length < 20) {
      return { action: 'HOLD', reason: 'Data candle kurang untuk Kronos', confidence: 50 };
    }

    const closes     = rawCandles.map(c => parseFloat(c[4])).reverse().slice(-60);
    const currentClose = closes[closes.length - 1];

    const res = await kronosHFRequest(closes);

    if (res.status === 503) {
      return { action: 'HOLD', reason: 'Kronos loading, tahan posisi dulu', confidence: 50 };
    }
    if (res.status !== 200) {
      return { action: 'HOLD', reason: 'HF API error, tahan posisi', confidence: 50 };
    }

    const predicted  = Array.isArray(res.body) ? res.body : [];
    if (predicted.length === 0) {
      return { action: 'HOLD', reason: 'Forecast kosong, tahan posisi', confidence: 50 };
    }

    const predCloses  = predicted.slice(0, 3).map(v => typeof v === 'number' ? v : parseFloat(v));
    const lastPred    = predCloses[predCloses.length - 1];
    const changePct   = (lastPred - currentClose) / currentClose * 100;
    const side        = (position.side || 'LONG').toUpperCase();
    const unrealized  = parseFloat(position.unrealizedPL || 0);

    // Momentum masih searah posisi?
    const momentumOk = side.includes('LONG') ? changePct > 0.1 : changePct < -0.1;

    if (!momentumOk && unrealized > 0) {
      return {
        action: 'CLOSE',
        reason: `Kronos prediksi momentum melemah (Δ ${changePct.toFixed(2)}%) — kunci profit $${unrealized.toFixed(2)}`,
        confidence: 80
      };
    }
    if (!momentumOk && unrealized < -0.2) {
      return {
        action: 'CLOSE',
        reason: `Kronos prediksi harga berlanjut melawan posisi ${side} (Δ ${changePct.toFixed(2)}%)`,
        confidence: 75
      };
    }
    return {
      action: 'HOLD',
      reason: `Kronos prediksi trend ${side} masih oke (Δ ${changePct.toFixed(2)}%), tahan posisi`,
      confidence: 70
    };
  } catch (e) {
    return { action: 'HOLD', reason: 'Kronos error: ' + e.message, confidence: 50 };
  }
}

// ============================================================
// TECHNICAL SIGNAL — rule-based scalping (baseline)
// ============================================================
...
// ============================================================
// GEMINI — Q&A + explain signal (tetap, peran tidak berubah)
// ============================================================
...
class NexusBot {
  constructor(config) {
    ...
    this.kronosAvailable = null; // null = belum dicek, true/false setelah healthcheck
  }

  // ── Cek apakah Kronos HF API tersedia ──
  async checkKronosHealth() {
    if (!HF_TOKEN) {
      this.kronosAvailable = false;
      this.log('SYS', 'Kronos: ⚠️ HUGGINGFACE_TOKEN belum diset — fallback ke Gemini');
      return;
    }
    try {
      // Kirim dummy request kecil untuk cek model status
      const res = await kronosHFRequest([1.0, 1.01, 0.99, 1.02, 1.0]);
      // 200 = ready, 503 = loading (masih ok, tinggal tunggu)
      this.kronosAvailable = res.status === 200 || res.status === 503;
      const statusText = res.status === 200 ? '✅ READY' : res.status === 503 ? '⏳ LOADING (model warm-up)' : `❌ ERROR ${res.status}`;
      this.log('SYS', `Kronos HuggingFace: ${statusText}`);
    } catch (e) {
      this.kronosAvailable = false;
      this.log('SYS', `Kronos HuggingFace: ⚠️ Tidak terjangkau — ${e.message}`);
    }
  }
  ...
  roundPrice(symbol, price) {
    ...
    const step = spec.priceEndStep;
    const decimals = spec.pricePlace;
  ...
  async runCycle() {
    ...
    try {
      ...
      // Step 1: Technical signal (cepat, tanpa API)
      ...
      this.log('AI', `★ BEST TEKNIKAL: ${best.market.symbol} ${best.signal} ${best.confidence}% — ${best.reasoning}`);

      // Step 2: Konfirmasi Kronos (menggantikan analyzeWithGemini sebagai entry filter)
      let finalSignal = { ...best };
      if (this.kronosAvailable !== false) {
        this.log('AI', `[KRONOS] Konfirmasi entry ${best.market.symbol}...`);
        const kronos = await kronosConfirmEntry(this.bitget, best.market);

        if (kronos.ok) {
          this.log('AI', `[KRONOS] ${best.market.symbol}: ${kronos.signal} ${kronos.confidence}% — ${kronos.reasoning}`);

          // ── Logika konfirmasi: Kronos HARUS setuju arah teknikal ──
          if (kronos.signal === best.signal) {
            // Setuju → gabungkan confidence (rata-rata, bobot 60% Kronos 40% teknikal)
            const blendedConf = Math.round(kronos.confidence * 0.6 + best.confidence * 0.4);
            finalSignal = {
              ...best,
              confidence: blendedConf,
              takeProfit: kronos.takeProfit > 0 ? kronos.takeProfit : best.takeProfit,
              stopLoss:   kronos.stopLoss   > 0 ? kronos.stopLoss   : best.stopLoss,
              reasoning:  `[Konfirmasi Kronos+Teknikal] ${kronos.reasoning}`,
              risk:       blendedConf >= 80 ? 'LOW' : blendedConf >= 65 ? 'MEDIUM' : 'HIGH',
              raw:        kronos.reasoning,
              kronosForecast: kronos.forecast
            };
            this.log('AI', `[KRONOS] ✅ Konfirmasi cocok — confidence gabungan ${blendedConf}%`);
          } else if (kronos.signal === 'WAIT') {
            // Kronos netral → turunkan confidence
            finalSignal.confidence = Math.round(best.confidence * 0.7);
            finalSignal.reasoning = `[Kronos WAIT, teknikal ${best.signal}] ${kronos.reasoning}`;
            this.log('AI', `[KRONOS] ⚠ Kronos WAIT — confidence diturunkan ke ${finalSignal.confidence}%`);
          } else {
            // Kronos BERLAWANAN → batalkan entry
            this.log('AI', `[KRONOS] ❌ Sinyal berlawanan (teknikal ${best.signal} vs Kronos ${kronos.signal}) — SKIP entry ${best.market.symbol}`);
            this.lastSignal = { ...best, kronosConflict: true, timestamp: Date.now() };
            return; // batalkan trade ini
          }
        } else {
          this.log('AI', `[KRONOS] Tidak tersedia: ${kronos.error} — pakai sinyal teknikal saja`);
          this.kronosAvailable = false;
        }
      }

      // Cek ulang confidence setelah Kronos
      if (finalSignal.confidence < minConf) {
        this.log('SYS', `Confidence setelah Kronos ${finalSignal.confidence}% < min ${minConf}% — SKIP`);
        return;
      }

      this.lastSignal = { ...finalSignal, timestamp: Date.now() };

      // Step 3: Gemini explain signal (async, non-blocking — peran tetap sama)
      if (this.config.geminiKey) {
        geminiExplainSignal(this.config.geminiKey, best.market, finalSignal)
          .then(text => {
            if (text) {
              this.lastSignal.raw = text;
              const analisa = text.match(/ANALISA:\s*([\s\S]+?)(?:ACTION:|$)/i)?.[1]?.trim();
              const action  = text.match(/ACTION:\s*(.+)/i)?.[1]?.trim();
              if (analisa) this.log('AI', `[GEMINI ANALISA ${best.market.symbol}] ${analisa.replace(/\n/g,' ')}`);
              if (action)  this.log('AI', `[GEMINI ACTION ${best.market.symbol}] ${action}`);
            }
          })
          .catch(() => {});
      }
      ...
      if (this.config.autoTrade) {
        await this.executeSignal(finalSignal, best.market);
      } else {
  ...
  async manageOpenPosition(position) {
    try {
      ...
      const profitLockPct = this.config.profitLockPct || 30;
      const lossLimitPct  = this.config.lossLimitPct  || 50;
      ...
      // ── EXIT CHECK: Kronos (menggantikan askAIToExit dengan Gemini) ──
      if (position.unrealizedPL > 0 && this.config.aiExitCheck !== false) {
        try {
          ...
          if (this.kronosAvailable !== false) {
            // Pakai Kronos untuk exit decision
            decision = await kronosExitDecision(this.bitget, position, market);
            this.log('AI', `[KRONOS EXIT] ${position.symbol}: ${decision.action} — ${decision.reason}`);
          } else {
            // Fallback ke Gemini jika Kronos offline
            decision = await this.askGeminiToExit(position, market);
            this.log('AI', `[GEMINI EXIT] ${position.symbol}: ${decision.action} — ${decision.reason}`);
          }

          if (decision.action === 'CLOSE') {
            await this.closePositionNow(position, this.kronosAvailable !== false ? 'KRONOS_EXIT' : 'AI_EXIT');
            return;
          }
        } catch (e) {
  ...
  // Fallback Gemini exit (dipakai hanya jika Kronos offline)
  async askGeminiToExit(position, market) {
...
REASON: max 1 sentence in Indonesian`;
  ...
  async executeSignal(signal, market) {
    try {
      ...
      if (spec && actualMargin > targetMargin * 1.2) {
        ...
        this.log('SYS', `⚠ ${market.symbol} min size ${spec.minTradeNum} → margin minimum $${minMarginNeeded.toFixed(2)}`);
      }
      ...
      let res = await this.bitget.placeOrder(market.symbol, side, attemptSize, leverage, tp, sl);
      ...
      if (res.code === '40762' || (res.msg || '').toLowerCase().includes('exceeds the balance')) {
        ...
        if (attemptSize > 0) {
          const retryMargin = (attemptSize * market.price) / leverage;
          this.log('ORDER', `Retry: Size ${attemptSize} | Margin ~$${retryMargin.toFixed(2)}`);
          res = await this.bitget.placeOrder(market.symbol, side, attemptSize, leverage, tp, sl);
        }
      }
      ...
      if (res.code === '00000') {
        ...
        this.log('ERR', `${market.symbol}: ${res.msg}`);
      } else if (res.code === '40762') {
        this.log('ERR', `${market.symbol}: Balance kurang. Saldo USDT-Futures mungkin <$${actualMargin.toFixed(2)}`);
      } else {
  ...
  rejectReversal() {
    if (!this.pendingReversal) return { ok: false, msg: 'Tidak ada reversal pending.' };
    const { pos } = this.pendingReversal;
  ...
  async start() {
    ...
    await this.checkKronosHealth();  // cek Kronos saat start
    await this.runCycle();
    ...
    // Re-check Kronos health tiap 5 menit
    this.kronosHealthInterval = setInterval(() => this.checkKronosHealth(), 5 * 60 * 1000);
  }
  ...
  stop() {
    ...
    clearInterval(this.kronosHealthInterval);
    this.reversalInterval = null;
  ...
  getStatus() {
    return {
      ...
      kronosStatus: this.kronosAvailable === null ? 'checking' : this.kronosAvailable ? 'online' : 'offline',
      config: {
...
JAWABAN:`;
  ...
  if (quotaError) {
    throw new Error('Gemini API quota habis hari ini. Tunggu reset besok atau upgrade plan di Google AI Studio.');
  }
...
// ============================================================
// GEMINI EXPLAIN — jelasin sinyal dalam bahasa manusia
// ============================================================
...
SINYAL: ${sig.signal} (confidence ${sig.confidence}%, risk ${sig.risk})
Alasan teknis: ${sig.reasoning}
Target TP: $${sig.takeProfit}, SL: $${sig.stopLoss}

Format respons (plain text, tanpa markdown):
SIGNAL: ${sig.signal}
CONFIDENCE: ${sig.confidence}%
RISK: ${sig.risk}
ANALISA: <3-4 baris penjelasan>
